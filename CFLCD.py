...
import pickle
import ctypes
from DrvCrystalfontz import ReadLCDMemory
...
class CFLCD:
	def __init__(self):
		self.set_window('lcd', gtk.glade.XML ( 'CF635.glade', "window_lcd"))
		self.windows['lcd'].signal_autoconnect(self)
		box = self.windows['lcd'].get_widget("vbox_lcd")
		darea = gtk.DrawingArea()
		darea.show()
		box.pack_start(darea, True, True, 0)
		self._('window_lcd').add_events(gtk.gdk.BUTTON_PRESS_MASK)
		self.lcd = LcdDisplay(darea)
		self.lcd.CreateGraphics()
		self.lcd.write_data2(0, 0, chr(177) + "BEXTA")
		self.add_signal('lcd-memory-ready', self.memory_ready)
		#self.add_func(self.get_lcd_memory)

	def lcd_show(self):
		self.set_current_window('lcd')
		self._('window_lcd').show()

	def memory_ready(self, visitor, memory):
		print "Memory ready"
		self.lcd.write_data2(0, 0, memory.line_one())
		self.lcd.write_data2(1, 0, memory.line_two())
		self.lcd.write_data2(2, 0, memory.line_three())
		self.lcd.write_data2(3, 0, memory.line_four())
		cgram = memory.cgram()
		print "Cgram Len", len(cgram)
		for char in range(0, 8):
			data = cgram[char*8:char*8+8]
			self.lcd.set_special_char(char, data)
				
		#if self.lcd.fontP: self.lcd.fontP.setup()
				
		self.set_current_window('lcd')
		self._('window_lcd').queue_draw()

	def on_window_lcd_delete_event(self, widget, data=None):
		widget.hide()
		return True

	def on_window_lcd_button_press_event(self, widget, data=None):
		self.set_current_window("lcd")
		self._('window_lcd').hide()

	#def get_lcd_memory(self, data=None):
	#	if self.current_module != -1:
	#		self.CF635.memory.fill_start()
...
class LcdDisplay(Generic, Text):
	...
	def CreateGraphics(self):
		...
		if not (self.disp_type & TWO_ROWS_IN_ONE):
			self.darea.set_size_request(self.cols*cw+2*border, self.rows*(ch+border)+border)
		else:
			self.darea.set_size_request(self.rows*self.cols*cw+2*border, ch+2*border)
	...
	def set_special_char(self, char, data):
		for row in range(0, 8):
			self.lcd_fonts[char][row] = ord(data[row])
		if self.fontP:
